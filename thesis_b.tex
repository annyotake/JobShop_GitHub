...
HPジョブは納期内に完了させ，
LPジョブの納期からの遅延日数と
作業者の残業時間を最小化することを目的とする．

\begin{equation}
	\begin{split}
	  F &= min \left\{\sum_{j|P_j = LP} \max \left[0, \left( \left\lceil\frac{\max\limits_i(pT_{ji})}{3\sigma}\right\rceil - \left\lceil\frac{E_j}{3\sigma}\right\rceil \right)\right]\right. \\
	    &\quad + \sum_{j|P_j = HP} \max \left[0, \left(\left\lceil\frac{E_j}{3\sigma}\right\rceil - \left\lceil\frac{\max\limits_i(pT_{ji})}{3\sigma}\right\rceil \right)\right] \cdot \left.\frac{E_j - \max\limits_i(pT_{ji})}{t^e_j} \right|_{t^e_j > 0} \\
			&\quad + \sum_w{\sum_d \max \left[0, \left( wt^w_d - \sigma - ot^w_d \right) \right]} \cdot H \\
		  &\quad + \left.\sum_{j|P_j = HP} \max \left[0, \left( \left\lceil\frac{\max\limits_i(pT_{ji})}{3\sigma}\right\rceil - \left\lceil\frac{E_j}{3\sigma}\right\rceil \right)\right] \cdot H + \sum_{j|P_j = LP} \left\lfloor \frac{t^e_j}{\sigma} \right\rfloor \cdot H \right\}
	\end{split}
\end{equation}
\\
...
\begin{equation}
	\min_i(sT_{ji}) \leq B_j, \forall j \in J
\end{equation}
  ...
  \begin{tabular}{ll}  \hline
    ...
    $j$ & ジョブの番号　$j \in J = \{1,2,\cdots,N_j\}$であり，$N_j$はジョブの数を表す．\\ \hline
		...
		$d$ & 日付　$d \in D = \{1,2,\cdots,N_d\}$であり，\\
		    & $N_d$は日数を表す．\\ \hline
		$wt^w_d$ & 作業者$w$の日付$d$における作業時間　シフトによって変わる． \\ \hline
		$\sigma$ & 標準シフトの分数　$\sigma = 8 \times 60 = 480$（分）とする． \\ \hline
		$ot^w_d$ & 作業者$w$が希望する日付$d$における残業時間 \\ \hline
		...
		$B_j$ & ジョブ$j$の開始時間 \\ \hline
		$E_j$ & ジョブ$j$の完了予定時間 \\ \hline
...
\centering
  ...
  \begin{tabular}{ll}  \hline
    用語 & 意味\\ \hline
		...
		$sT_{ji}$ & 作業$O_{ji}$の準備開始時間 \\ \hline
		$cT_{ji}$ & 作業$O_{ji}$の準備完了時間 \\ \hline
		$pT_{ji}$ & 作業$O_{ji}$の実行完了時間 \\ \hline
