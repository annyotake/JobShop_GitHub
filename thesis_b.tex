...
\chapter{問題設定}
% $t^e_j$の休日における計算式、休日はwt = otである制約、第４・５項は提案解法に回す

本研究で対象とするFJSPでは，
生産現場で行うジョブの集合を$J$，
作業の集合を$O$とし，
各ジョブは集合$O$を含むものとする．
そして，各ジョブは異なる種類の作業を含むことができるが，
作業は所定の順番で完了しなければならない．
また，各ジョブに優先度が設けられており，
高優先度のHPジョブと低優先度のLPジョブに分けられ，
HPジョブは納期内に完了しなければならない．
一方，LPジョブは納期内に完了しないことを許容する．
ここで対象とする資源は機械と作業者の２つであり，
機械の集合を$M$，作業者の集合を$W$とする．
各ジョブの作業は，機械と作業者または作業者のみによって実行される．
上記の仮定を踏まえて，
本研究で対象とするFJSPにおける制約を以下に示す．
...
(8) 標準シフトは8時～17時の9時間とする．
    また，作業者の勤務時間は休憩時間などの作業をしていない時間も含む．
...
目的関数$F$の第3項では
残業時間が作業者の希望に沿っていない場合に
ペナルティ係数$H$を乗じた数値を目的関数$F$に加える．
また，式(4.2)の$t^e_j$はジョブ$j$における作業者の合計残業時間を表す．

\begin{equation}
	\begin{split}
	  F &= min \Bigg\{\sum_{j|P_j = LP} \max \left[0, \left( \left\lceil\frac{\max\limits_i(pT_{ji})}{1440}\right\rceil - \left\lceil\frac{E_j}{1440}\right\rceil \right)\right] \\
	    &\quad + \sum_{j|P_j = HP} \max \left[0, \left(\left\lceil\frac{E_j}{1440}\right\rceil - \left\lceil\frac{\max\limits_i(pT_{ji})}{1440}\right\rceil \right)\right] \cdot \left.\frac{E_j - \max\limits_i(pT_{ji})}{t^e_j} \right|_{t^e_j > 0} \\
			&\quad + \sum_w{\sum_d \max \left[0, \left( wt^w_d - \sigma - ot^w_d \right) \right]} \cdot H \Bigg\}
	\end{split}
\end{equation}

\begin{equation}
  t^e_j = \sum_i{\sum_w{\sum^{\left\lceil\frac{pT_{ji}}{3\sigma}\right\rceil}_{d = \left\lceil\frac{sT_{ji}}{3\sigma}\right\rceil}{x^w_{ji} \cdot \left( wt^w_d - \sigma \right)}}}
\end{equation}
\\
...
制約条件(4.5)は，各HPジョブの残業時間はその納期内に完了できる場合の余った時間以上でなければならないことを表す．
                そうでない場合はそのHPジョブは残業せずに納期内に完了できることを意味する．
...
制約条件(4.13)は，すべての機械と作業者は同時に２つ以上の作業を処理することはできないことを表す．
制約条件(4.14)は，$x^m_{ji}, x^w_{ji}, y^m_{j^{\prime}i^{\prime}ji}, y^w_{j^{\prime}i^{\prime}ji}$は
                 いずれも2値変数であることを表す．

\begin{equation}
  t^e_j = 0, \forall j \in \{j|P_j = LP\}
\end{equation}
...
\begin{equation}
	\begin{split}
	 ...
	 \begin{cases}
		 ...
		 sT_{ji}, cT_{ji}, pT_{ji} \notin [B_{mk_2}, E_{mk_2}] & \text{if $x^m_{ji} = 1$} \\
	 \end{cases} \\[-10pt]
...
\begin{equation}
	x^m_{ji}, x^w_{ji}, y^m_{j^{\prime}i^{\prime}ji}, y^w_{j^{\prime}i^{\prime}ji} \in \{0, 1\}, \forall i,i^{\prime} \in I, j,j^{\prime} \in J, m \in M, w \in W
\end{equation}
...
\centering
  ...
  \begin{tabular}{ll}  \hline
    ...
    $j$ & ジョブの番号　$j \in J = \{1,2,\cdots,N_j\}$であり，$N_j$はジョブの数を表す．\\ \hline
		...
		$ot^w_d$ & 日付$d$における作業者$w$が希望する残業時間 \\ \hline
		...
		$[B_{wk}, E_{wk}]$ & 作業者$w$が作業をしていないまたは他の作業をしている期間 \\
		                   & $k \in K_w = \{1,2,\cdots,N^k_w\}$であり，\\
		                   & $N^k_w$は作業者$w$が作業をしていない期間の数を表す．\\ \hline
  \end{tabular}
\end{table}
...
第１段階では，IDEと同様に
生産システムにおけるスケジューリングに焦点を当てる．
ただし，ここでは全作業者の１日のシフトを
標準シフトの９時間（休憩時間などの作業をしていない時間も含む）とする．
また，作業者は残業を行わないものとする．
この条件下で，DEを用いて
作業間の資源を割り当てる最適な順番と
各作業に割り当てる最適な資源を求める．
...
その納期からの遅延期間にペナルティ係数$H$を乗じた数値を目的関数$F$に加える．
そして，このDEによって目的関数が最良である個体群を得る．

第２段階では，第１段階で得られた目的関数が最良である個体群において，
HPジョブが納期内に完了しない場合，
HPジョブが納期内に完了するよう作業者に残業を割り当てる．
このとき，あらかじめ定められている作業者が希望する残業時間に基づいて，
作業者の希望に沿うよう残業を割り当てる．
また，LPジョブを担当する作業者に残業が割り当てられた場合，
その残業は不要であるので，
その残業日数にペナルティ係数$H$を乗じた数値を目的関数に$F$に加える．
よって，目的関数$F$は式(5.1)の目的関数$F^{\prime}$に変更され，
この目的関数$F^{\prime}$の下でスケジューリングを行う．

\begin{equation}
	\begin{split}
	  F^{\prime} &= min \left\{\sum_{j|P_j = LP} \max \left[0, \left( \left\lceil\frac{\max\limits_i(pT_{ji})}{1440}\right\rceil - \left\lceil\frac{E_j}{1440}\right\rceil \right)\right]\right. \\
	             &\quad + \sum_{j|P_j = HP} \max \left[0, \left(\left\lceil\frac{E_j}{1440}\right\rceil - \left\lceil\frac{\max\limits_i(pT_{ji})}{1440}\right\rceil \right)\right] \cdot \left.\frac{E_j - \max\limits_i(pT_{ji})}{t^e_j} \right|_{t^e_j > 0} \\
			         &\quad + \sum_w{\sum_d \max \left[0, \left( wt^w_d - \sigma - ot^w_d \right) \right]} \cdot H \\
		           &\quad + \left.\sum_{j|P_j = HP} \max \left[0, \left( \left\lceil\frac{\max\limits_i(pT_{ji})}{1440}\right\rceil - \left\lceil\frac{E_j}{1440}\right\rceil \right)\right] \cdot H + \sum_{j|P_j = LP} \left\lfloor \frac{t^e_j}{\sigma} \right\rfloor \cdot H \right\}
	\end{split}
\end{equation}
\\
